...
import { isPracticeMode } from '../hooks/usePracticeSession';
...
export const ReviewPracticeView = ({ userId }: ReviewPracticeViewProps) => {
  ...
  if (!session.currentTask) {
    return <div className="app-viewport flex items-center justify-center bg-sand-50 px-6 text-center text-lg text-ink-900">No review cards available.</div>;
  }

  const navigateToResults = (result: SessionResult) => {
    navigate('/results', {
      state: {
        result,
        review: true,
  ...
  const handleFlashcardGrade = async (correct: boolean) => {
    if (!session.currentTask || !mode || submitting) {
      return;
    }
    ...
    const nextWrongAnswers = correct
      ...
      : [
          ...flashcardWrongAnswers,
          {
            ...
            prompt: `${session.currentTask.topicName}: ${promptText}`,
  ...
  const handleWriteInSubmit = () => {
    ...
    if (correct) {
      ...
      setWriteInWrongAnswers((current) => [
        ...
        {
          taskId: session.currentTask.id,
          prompt: `${session.currentTask.topicName}: ${session.currentTask.front}`,
          userAnswer: answer,
  ...
  return (
    <div className="app-viewport flex flex-col bg-sand-50">
      <div className="flex items-start justify-between px-6 pb-4 pt-6">
        <Link className="text-2xl font-semibold text-forest-700" to="/review/modes">
          Back
        </Link>
      </div>
